...
public class AppointmentController {
	...
	@GetMapping("/appointment/getAppointment/{date}")
	public ResponseEntity<List<Appointment>> fetchAppointmentByDate(@PathVariable("date") String date) 
	{
		LocalDate date1 = LocalDate.parse(date);
		List<Appointment> appointment = apptService.getAppointmentByDate(date1);
